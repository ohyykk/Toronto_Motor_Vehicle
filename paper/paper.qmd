...
ggplot(premises_summary, aes(x = reorder(premises_type, -theft_count), y = theft_count)) +
  ...
  theme_minimal() +
  labs(
    title = "Theft Incidents by Premises Type",
    x = "Premises Type",
    y = "Number of Theft Incidents"
  ) +
  theme(
...
ggplot(road_surface_summary, aes(x = reorder(road_conditions, -incident_count), y = incident_count)) +
  ...
  theme_minimal() +
  labs(
    title = "Incidents by Road Surface Condition",
...
#| fig-cap: "5 Examples of Different Times of Day"
#| echo: false
#| warning: false
#| message: false
...
# Select 5 random examples of time_of_day
time_of_day_examples <- cleaned_traffic_data %>%
  select(time_of_day) %>%
  distinct() %>%               # Ensure unique times
  sample_n(5)                  # Randomly select 5 examples

# Create the table with a frame
time_of_day_examples %>%
  kable(
    col.names = c("Time of Day"),
    align = "c"
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "bordered"),
...
#### Visibility
The Visibility variable is a categorical predictor that describes the environmental visibility conditions at the time of a collision, such as clear, foggy, or reduced due to rain or snow. This variable is critical for assessing how different visibility levels impact the likelihood of collisions. Poor visibility conditions, such as heavy fog, snow, or rain, can obscure drivers’ ability to perceive road hazards or other vehicles, increasing the probability of incidents. Conversely, clear visibility conditions often correlate with lower collision risks.
...
plot_dark <- ggplot(merged_data %>% filter(lighting_condition_group == "Dark"), aes(x = risk_index, fill = lighting_condition_group)) +
  ...
  labs(x = "Risk Index", y = "Density", title = "Risk Index for Dark Conditions") +
  theme_minimal() +
  theme(legend.position = "none")
...
plot_daylight <- ggplot(merged_data %>% filter(lighting_condition_group == "Daylight"), aes(x = risk_index, fill = lighting_condition_group)) +
  geom_density(alpha = 0.6) +
  scale_fill_manual(values = c("Daylight" = "#FFA07A")) +
  labs(x = "Risk Index", y = "Density", title = "Risk Index for Daylight Conditions") +
  theme_minimal() +
  theme(legend.position = "none")
...
plot_dawn <- ggplot(merged_data %>% filter(lighting_condition_group == "Dawn"), aes(x = risk_index, fill = lighting_condition_group)) +
  geom_density(alpha = 0.6) +
  scale_fill_manual(values = c("Dawn" = "#87CEFA")) +
  labs(x = "Risk Index", y = "Density", title = "Risk Index for Dawn Conditions") +
  theme_minimal() +
  theme(legend.position = "none")
...
plot_artificial <- ggplot(merged_data %>% filter(lighting_condition_group == "Artificial Lighting"), aes(x = risk_index, fill = lighting_condition_group)) +
  geom_density(alpha = 0.6) +
  scale_fill_manual(values = c("Artificial Lighting" = "#A9A9A9")) +
  labs(x = "Risk Index", y = "Density", title = "Risk Index for Artificial Lighting Conditions") +
  theme_minimal() +
  theme(legend.position = "none")
