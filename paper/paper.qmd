...
This project is motivated and guided by Rohan Alexander and his book [@tellingstories]. Data used in this paper was cleaned, analyzed and modeled with the programming language R [@citeR]. Also with support of additional packages in R: `readr` [@citeReadr], `ggplot2` [@citeGgplot2], `tidyverse` [@citeTidyverse], `dplyr` [@citeDplyr], `here` [@citeHere], `knitr` [@citeKnitr], `kableExtra` [@citeKableExtra], `palmerpenguins` [@citePalmerpenguins], `performance` [@citePerformance], `tidygraph` [@citeTidygraph], `ggraph` [@citeGgraph], `lubridate` [@citeLubridate], `sf` [@citeSf], `osmdata` [@citeOsmdata], `car` [@citeCar],
...
The **Road Surface Condition** variable serves as a categorical predictor, detailing the state of the road at the time of an incident. Categories include dry, wet, loose snow, slush, ice, packed snow, and others. This variable enables the analysis to assess how different surface conditions contribute to motorbike collision risks.

@fig-road_surface_summary illustrates the number of incidents under each road surface condition, showing that most incidents occur on dry roads, followed by wet roads. Conditions such as loose snow, ice, and packed snow account for a smaller proportion of incidents, reflecting their lower occurrence frequency. However, these different surface conditions may represent a disproportionately high risk per unit of exposure. This distribution underscores the importance of accounting for surface conditions in collision prevention strategies, especially during adverse weather, to enhance road safety and reduce risks.
...
#| tbl-cap: "5 Examples of Different Times of Day with Incident Count"
#| echo: false
#| warning: false
#| message: false
...
#| fig-subcap: ["Low Light (Dark)", "Dawn Light", "Bright Daylight and Artificial Light", "Artificial Lighting Conditions"]
#| echo: false
#| warning: false
#| message: false
#| fig-width: 8.5
...
In constructing the theft risk sub-indexes, the choice of predictor variables: Hour of the Day, Day of the Week, and Premises Type was informed by both theoretical and practical considerations rooted in spatial, temporal, and environmental factors associated with theft incidents. These variables were selected from the cleaned theft dataset to create a strong and interpretable model that captures key patterns of theft occurrences while aligning with the study's goal of identifying targeted interventions.

1. Hour of the Day

The inclusion of the Hour of the Day as a predictor variable reflects the strong temporal dependency of theft occurrences. Theft risk often varies significantly across a 24-hour period due to human activity patterns, such as peak commuting times, work hours, and late-night inactivity. For example, motor vehicle thefts are more likely to occur during nighttime hours when there is reduced surveillance and fewer bystanders to deter criminal behavior. By incorporating the Hour of the Day into the theft risk sub-index, the model accounts for this temporal dimension, enabling stakeholders to identify high-risk hours and implement time-specific interventions, such as increased nighttime patrols or surveillance in parking lots and residential areas.

2. Day of the Week

The Day of the Week was selected to capture the variations in theft risk across different days, which often reflect behavioral patterns linked to work, leisure, and mobility. Weekends, for instance, may see heightened theft activity in areas with nightlife or during events that attract large crowds, while weekdays might exhibit theft patterns associated with commuter parking zones or commercial districts. The inclusion of this variable allows the model to identify day-specific risks and tailor interventions, such as targeted surveillance during high-risk days or public awareness campaigns about theft prevention.

3. Premises Type

Premises Type represents the spatial dimension of theft risk, capturing the environmental characteristics where thefts are most likely to occur. For example, thefts in residential areas (e.g., houses, apartments) often occur when vehicles are parked in driveways or on the street with limited security measures. Similarly, commercial areas and outdoor public spaces may pose unique risks due to their high foot traffic, limited oversight during certain hours, or accessibility of parked vehicles. Including Premises Type in the sub-index allows the model to differentiate risk levels based on spatial characteristics and recommend location-specific strategies, such as installing better lighting in outdoor parking areas or promoting secure storage options for residential settings.


### Collision Probability

The predictors chosen for the logistic regression model were carefully selected to capture key spatial, temporal, and environmental factors that influence the likelihood of severe collisions. These variables were derived from the cleaned collision dataset, ensuring data quality and relevance to the study's focus on urban mobility and safety in Toronto.

1. Neighborhood ID 

The inclusion of hood_158, a unique identifier for neighborhoods, allows the model to capture spatial patterns in collision occurrences. Different neighborhoods may exhibit varying levels of collision risk due to factors such as traffic density, road design, infrastructure quality, and socioeconomic conditions. By including this variable, the model can account for geographic heterogeneity, identifying high-risk areas and enabling targeted interventions such as infrastructure improvements or traffic calming measures in specific neighborhoods.

2. Road Surface Condition 

Road surface conditions, such as dry, wet, or icy surfaces, significantly impact vehicle control and braking effectiveness, making this variable a critical predictor. Collisions are more likely under adverse surface conditions, such as rain, snow, or ice, due to reduced traction and visibility. Including this variable in the model ensures that environmental factors influencing vehicle dynamics are accounted for, providing insights into how weather and road maintenance contribute to collision risk. This information can guide policies such as improved road drainage systems or timely snow and ice removal.

3. Lighting Condition 

Lighting conditions, which describe whether collisions occur during daylight, nighttime, or under artificial light, are essential for understanding the role of visibility in collisions. Poor lighting can reduce drivers' ability to perceive hazards, increasing the likelihood of accidents. By incorporating this variable, the model can assess how lighting contributes to collision risk, highlighting the importance of enhanced street lighting in poorly lit areas or during nighttime hours.
\newpage

4. Traffic Control 

The presence and type of traffic control devices, such as stop signs, traffic signals, and pedestrian crossovers, play a important role in preventing collisions by regulating vehicle flow and ensuring predictable driver behavior. The inclusion of this variable allows the model to evaluate the effectiveness of existing traffic control measures and identify gaps, such as intersections without adequate control devices, where collisions are more likely to occur. This idea can inform infrastructure upgrades and the installation of additional traffic management systems.

5. Road Visibility 

Road visibility conditions, influenced by factors like fog, rain, or snow, are directly linked to drivers' ability to perceive their surroundings and react to potential hazards. Poor visibility increases the risk of collisions by reducing reaction time and decision-making effectiveness. Including this variable ensures that the model captures the impact of adverse weather and environmental conditions on collision likelihood, emphasizing the need for weather-responsive traffic management strategies, such as variable speed limits or enhanced signage.

6. Time of Day

The Time of Day is a key temporal variable that captures variations in collision risk throughout a 24-hour period. Collision likelihood often peaks during specific hours, such as morning and evening rush hours, due to increased traffic volume, or late at night, when factors like fatigue or impaired driving may come into play. Including this variable allows the model to account for temporal patterns in collision occurrences, supporting the implementation of time-specific measures, such as increased traffic enforcement during high-risk hours or adjustments to traffic light timings.
...
The survey is structured into six main sections, each designed to gather detailed information about factors influencing motorbike risks.

**1. Demographics**

This section collects essential background information to contextualize the respondent's behaviors and risks:
...
- Motorbike ownership status

**2. Motorbike Usage Patterns**

Questions in this section aim to understand how respondents use their motorbikes:

- Frequency of use (e.g., daily, weekly, monthly)
- Primary purpose (e.g., commuting, recreation, delivery work)
- Average trip duration
- Type of roads frequently used (e.g., highways, city streets, rural roads)
- Riding seasonality (e.g., year-round, summer-only)

**3. Riding Behavior**

This section looks into behaviors that may influence collision and theft risks:

- Average speed during rides
- Use of safety gear (e.g., helmets, gloves, reflective clothing)
- Adherence to traffic laws (e.g., stopping at signals, avoiding speeding)
- Riding under adverse conditions (e.g., rain, low light)
- Group riding versus solo riding

**4. Situational Risk Factors**

This section explores external factors that contribute to motorbike risks:

- Time of day most frequently ridden (morning, afternoon, night)
- Common weather conditions experienced (e.g., sunny, rainy, foggy)
- Road surface conditions during rides (e.g., dry, wet, icy)
- Presence of traffic control measures on roads used
- Experience with poorly lit or visibility-compromised areas

**5. Risk Awareness and Experience**

This section assesses respondents' awareness of risks and their personal experiences:

- Perception of collision and theft risks in their area
- Personal experience with motorbike theft (e.g., number of thefts, recovered cases)
- Personal experience with collisions (e.g., frequency, severity, injuries sustained)
- Awareness of high-risk areas for theft or collisions
- Perception of environmental risks (e.g., poor road design, lack of lighting)

**6. Preventive Measures and Interventions**

This section investigates actions taken by riders to mitigate risks and their opinions on potential interventions:

- Security measures used to prevent theft (e.g., locks, alarms, GPS trackers)
- Riding adjustments to avoid collisions (e.g., avoiding high-risk areas, reducing     speed)
- Willingness to participate in safety programs (e.g., high-level riding courses)
  Opinions on public safety measures (e.g., improved lighting, better road   
  infrastructure
- Suggestions for reducing theft and collisions in their area


### Question Types

To ensure the survey collects meaningful, diverse, and actionable data, a combination of question types will be used. Each type serves a specific purpose, enabling both quantitative analysis and qualitative understanding. Below are the key question types included in the survey:
...
Drop-down questions are useful for limiting the display of options while allowing respondents to choose from a broad range of answers.

7. Conditional/Follow-Up Questions

Conditional questions are used to customize the survey based on previous responses, ensuring relevance and efficiency.
...
**Motorbike Usage Patterns**

3. How frequently do you use your motorbike?
...
4. How important are the following reasons for riding a motorbike to you?
...
(Rank each reason: Not important at all, Slightly important, Moderately important, Very important, Extremely important)


**Riding Behavior**

5. How often do you wear a helmet while riding?
...
6. What is your average speed while riding?
...
7. What time of day do you most often ride?
...
8. How often do you encounter poor road visibility during rides?
...
**Risk Awareness and Experience**

9. Have you ever had your motorbike stolen?

- Yes 

- No

(If "Yes," follow-up: How many times has your motorbike been stolen?)
...
10. How would you rate the risk of collisions in your area?
...
11. How important are the following factors for motorbike safety?
...
(Rate each factor: Not important at all, Slightly important, Moderately important, Very important, Extremely important)

**Preventive Measures**

12. What security measures do you currently use to prevent motorbike theft?
...
13. How effective do you think these changes would be in reducing motorbike collisions in your area?

(Rate each factor: Not effective at all, Slightly effective, Moderately effective, Very effective, Extremely effective)

- Improved road lighting

-Better road maintenance (e.g., fixing potholes, clearing debris)

- Increased police patrols in high-risk areas

- Public education on motorbike safety

- Enforcing speed limits

14. What changes do you think would most effectively reduce motorbike collisions in your area?
(Open-ended)
